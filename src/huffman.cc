...
namespace huffman {
  ...
  HuffmanTree *build_tree(vector< pair<char, float> > &alph) {
    ...
    for (vector< pair<char, float> >::iterator it = alph.begin();
        it != alph.end(); ++it) {
  ...
  vector< pair<char, float> > make_freq_table(string inp) {
    map<char, unsigned> cfmap;
    vector< pair<char, float> >cfvec;
...
using namespace huffman;
  int main() {
    for (unsigned i = 0; i < sizeof(tests)/sizeof(tests[0]); i++) {
      string s = tests[i];
      cout << "\n>>>>>>> test " << i << " >>>>>>>" << endl;
      vector< pair<char, float> > cfvec = huffman::make_freq_table(s);
      huffman::HuffmanTree *htree = huffman::build_tree(cfvec);
      //print_tree(htree);
      huffman::codetable ctbl = huffman::build_lookup_table(htree);
      huffman::code_t t = huffman::encode(s, ctbl);
      cout << "original:" << endl << s << endl;
      cout << "encoded (compression ratio: "
        << (t.size() + 7) / 8 << "/" << s.size() << " or "
        << ((float)(t.size() / 8) / s.size()) << "):" << endl;
      string packed = huffman::bitvec_to_string(t);
      huffman::hexdump((unsigned char *)(packed.c_str()), packed.size());

      // Decode
      code_t t1 = huffman::string_to_bitvec(packed);
      assert(std::equal(t.begin(), t.end(), t1.begin()));
      string s1 = huffman::decode(t1, htree);
      //cout << "decoded:\n" << s1 << endl;
      assert(s1 == s);
      delete htree;
    }
